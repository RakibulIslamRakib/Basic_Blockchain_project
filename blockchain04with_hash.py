...
def get_balance(participant):
    ...
    amount_sent = functools.reduce(lambda tx_sum, tx_amm: tx_sum + sum(tx_amm) if len(tx_amm) > 0 else tx_sum + 0, tx_sender, 0 )

    tx_recipient = [[tx['amount'] for tx in block['transactions']
                     if tx['recipient'] == participant] for block in blockchain]
    amount_received = functools.reduce(lambda tx_sum, tx_amm: tx_sum + sum(tx_amm) if len(tx_amm) > 0 else tx_sum + 0, tx_recipient, 0 )
